...
void LCD_WR_REG ( u16 data ) { /* 写寄存器函数 */
    LCD_RS_CLR;
    LCD_CS_CLR;
    DATAOUT ( data );
    LCD_WR_CLR;
...
void LCD_WR_DATAX ( u16 data ) { /* 写数据函数 */
    LCD_RS_SET;
    LCD_CS_CLR;
    DATAOUT ( data );
    LCD_WR_CLR;
...
u16 LCD_RD_DATA ( void ) { /* 读LCD数据，返回读到的值 */
    u16 t;
    GPIOB->CRL = 0X88888888; /* PB0至7是上拉输入 */
    GPIOB->CRH = 0X88888888; /* PB8至15是上拉输入 */
    ...
    LCD_RD_SET;
    LCD_CS_SET;
    GPIOB->CRL = 0X33333333; /* PB0至7是上拉输出 */
    GPIOB->CRH = 0X33333333; /* PB8至15是上拉输出 */
    GPIOB->ODR = 0XFFFF; /* 全部输出高 */
    return t;
}

void LCD_WriteReg ( u16 LCD_Reg, u16 LCD_RegValue ) { /* 写寄存器。参数LCD_Reg是寄存器编号，LCD_RegValue是要写入的值 */
    LCD_WR_REG ( LCD_Reg );
    LCD_WR_DATA ( LCD_RegValue );
}

u16 LCD_ReadReg ( u16 LCD_Reg ) { /* 读寄存器，参数LCD_Reg是寄存器编号，返回读到的值 */
...
void LCD_WriteRAM ( u16 RGB_Code ) { /* LCD写GRAM，RGB_Code是颜色值 */
    LCD_WR_DATA ( RGB_Code );
}

u16 LCD_BGR2RGB ( u16 c ) { /* 从ILI93xx读出的数据为GBR格式，而写入时为RGB格式，通过该函数转换。参数c是GBR格式的颜色值 */
    u16  r, g, b, rgb;
    b = ( c >> 0 ) & 0x1f;
    g = ( c >> 5 ) & 0x3f;
    r = ( c >> 11 ) & 0x1f;
...
u16 LCD_ReadPoint ( u16 x, u16 y ) { /* 读取个某点的颜色值。参数x和y是坐标 */
    u16 r, g, b;
    ...
    if ( lcddev.id == 0X9341 || lcddev.id == 0X6804 || lcddev.id == 0X5310 ) {
        LCD_WR_REG ( 0X2E ); /* 向9341、6804和5310发送读GRAM指令 */
    } else if ( lcddev.id == 0X5510 ) {
        LCD_WR_REG ( 0X2E00 ); /* 向5510发送读GRAM指令 */
    } else {
        LCD_WR_REG ( R34 ); /* 向其他IC发送读GRAM指令 */
    }
...
void LCD_SetCursor ( u16 Xpos, u16 Ypos ) { /* 设置光标位置，参数Xpos是横坐标，Ypos是纵坐标 */
    if ( lcddev.id == 0X9341 || lcddev.id == 0X5310 ) {
        ...
        if ( lcddev.dir == 1 ) {
            Xpos = lcddev.width - 1 - Xpos; /* 横屏其实就是调转x和y坐标 */
        }
...
/* 设置LCD的自动扫描方向，注意，其他函数可能会受到此函数设置的影响(尤其是9341和6804)，所以一般设置为L2R_U2D。
   如果设置为其他扫描方式，可能导致显示不正常。参数dir为0至7，代表8个方向。9320、9325、9328、4531、4535、
   1505、b505、8989、5408、9341、5310、5510等IC已经实际测试 */
void LCD_Scan_Dir ( u8 dir ) {
    ...
    if ( lcddev.id == 0x9341 || lcddev.id == 0X6804 || lcddev.id == 0X5310 || lcddev.id == 0X5510 ) { /* 9341、6804、5310和5510很特殊 */
        switch ( dir ) {
...
void LCD_DrawPoint ( u16 x, u16 y ) { /* 画点，参数x和y是坐标 */
    LCD_SetCursor ( x, y ); /* 设置光标位置 */
    LCD_WriteRAM_Prepare(); /* 开始写入GRAM */
    LCD_WR_DATA ( POINT_COLOR ); /* 此点的颜色 */
}

void LCD_Fast_DrawPoint ( u16 x, u16 y, u16 color ) { /* 快速画点，参数x和y是坐标，color是颜色 */
    if ( lcddev.id == 0X9341 || lcddev.id == 0X5310 ) {
        LCD_WR_REG ( lcddev.setxcmd );
        LCD_WR_DATA ( x >> 8 );
        LCD_WR_DATA ( x & 0XFF );
        ...
        if ( lcddev.dir == 1 ) {
            x = lcddev.width - 1 - x; /* 横屏其实就是调转x和y坐标 */
        }
...
void LCD_Display_Dir ( u8 dir ) { /* 设置LCD显示方向。参数dir为0是竖屏，1是横屏 */
    if ( dir == 0 ) {
        lcddev.dir = 0;
        lcddev.width = 240;
        lcddev.height = 320;
...
/* 设置窗口，并设置画点坐标到窗口左上角(sx,sy)。参数sx和sy是窗口起始坐标(左上角)；width和height是窗口宽度和高度；68042横屏时不支持窗口设置 */
...
void LCD_Init ( void ) { /* 初始化lcd，该函数可以初始化各种ALIENTEK的LCD液晶屏 */
...
void LCD_Clear ( u16 color ) { /* 清屏函数，参数color是清屏的填充色 */
    u32 index = 0;
...
void LCD_Fill ( u16 sx, u16 sy, u16 ex, u16 ey, u16 color ) { /* 在指定区域内填充指定颜色，参数color是填充的颜色 */
    u16 i, j;
    u16 xlen = 0;
    u16 temp;
...
void LCD_Color_Fill ( u16 sx, u16 sy, u16 ex, u16 ey, u16 *color ) { /* 在指定区域内填充指定颜色块，参数color是填充的颜色 */
    u16 height, width;
    u16 i, j;
...
void LCD_DrawLine ( u16 x1, u16 y1, u16 x2, u16 y2 ) { /* 画线。参数x1和y1是起点坐标，x2和y2是终点坐标 */
    u16 t;
...
void LCD_Draw_Circle ( u16 x0, u16 y0, u8 r ) { /* 画圆。参数x和y是中心点，r是半径 */
    int a, b;
    int di;
    a = 0;
    b = r;
...
/* 在指定位置显示一个字符，参数x和y是起始坐标，num是要显示的字符，size是字体大小(12/16/24)，mode设置叠加(1)或非叠加(0) */
...
/* 显示数字，高位的0不显示。参数x和y是起点坐标，len是数字的位数，size是字体大小，color是颜色，num是数值 */
...
/* 显示数字，高位为0时还显示0。参数x和y是起点坐标，num是数值(0~999999999)，len是长度(即要显示的位数)，size是字体大小；
   对于mode，[7]为0时不填充，为1时填充0，[6:1]保留，[0]为0时非叠加显示，为1时叠加显示 */
...
/* 显示字符串。参数x和y是起点坐标，width和height是区域大小，size是字体大小，p是字符串起始地址 */
